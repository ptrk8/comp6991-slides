...
* Do threads execute in parallel or concurrently? <!-- On operating systems that map user threads to kernel threads, which is pretty much all operating systems, threads execute in parallel.  -->

---

# How to wait for all threads to finish using `join` handles
