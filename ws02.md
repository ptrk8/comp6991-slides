...
* Rust's standard library Hashmap implementation is *really* fast.
...
* Do you notice something interesting/missing in the declaration of `book_reviews`? 👀
...
* The three derive macros we'll focus on will be:
    * `Debug`
    * `Copy`
    * `Clone`
...
* `println` has no idea *how* to print out the struct.

---

# Solution

* Implement the `std::fmt::Debug` trait, which defines how the struct should be formatted when using the `{:?}` format specifier.
* Now we can now print out our struct.
